...
def parse_records(records):
	...
	for record in records:
		result.append([a for a in re.compile("(\s)").split(record) if a.strip() != ''])
	return result

def aggregate_per_guard(records):
	guard_data = {}
	for record in records:
		guard_id = record[1]
		date = record[0]
		if guard_id not in guard_data:
			guard_data[guard_id] = {}
		if date not in guard_data[guard_id]:
			guard_data[guard_id][date] = []
		for point in list(record[2]):
			guard_data[guard_id][date].append(point)
	return guard_data

def get_who_slept_most(guard_data):
	sleep_time = {}
	for key in guard_data:
		sleep_time[key] = 0
	for guard, days in guard_data.iteritems():
		for day, points in days.iteritems():
			sleep_time[guard] += points.count('#')
	return max(sleep_time.iteritems(), key=operator.itemgetter(1))[0]

def get_sleepiest_minute(sleepiest_guard_data):
	count_per_minute = {}
	for i in range (0, 59):
		count_per_minute[i] = 0
	for day, points in sleepiest_guard_data.iteritems():
		for i in range (0, 59):
			count_per_minute[i] += 1 if points[i] == '#' else 0
	return max(count_per_minute.iteritems(), key=operator.itemgetter(1))[0]
...
# Clean raw input and aggregate per day
clean_input = [a for a in raw_input.replace('] ', '  ').replace('[1518-', '').replace('Guard ', '').replace('falls ', '').replace(' up', '').replace(' shift', '').split('\n') if a.strip() != '']
clean_input.sort()
input_per_day = {}
current_guard = '#NULL'
for row in clean_input:
	date_action = row.split('  ')
	date = date_action[0][0:5]
	time = date_action[0][6:11].strip()
	action = date_action[1]
	if '#' in action:
		action_parts = action.split(' ')
		current_guard = action_parts[0]
	if date not in input_per_day:
		input_per_day[date] = []
	input_per_day[date].append((time, action, current_guard))

# Transform cleaned input into data records (e.g. "11-01  #10  .....####################.....#########################.....")
data = []
for day, actions in input_per_day.iteritems():
	guard_sleep_times = {}
	for action_triple in actions:
		time, action, guard = action_triple
		if guard not in guard_sleep_times:
			guard_sleep_times[guard] = ([], ['.' for a in range(0, 60)]) # (sleep sessions, ascii points)
		if time[0:3] == '00:' and action == 'asleep':
			guard_sleep_times[guard][0].append((int(time.split(':')[1]), None))
		elif time[0:3] == '00:' and action == 'wakes':
			if len(guard_sleep_times[guard][0]) > 0 and guard_sleep_times[guard][0][-1][1] is None:
				guard_sleep_times[guard][0][-1] = (guard_sleep_times[guard][0][-1][0], int(time.split(':')[1]))
	
	for guard in guard_sleep_times:
		points = guard_sleep_times[guard][1]
		for sleep_session in guard_sleep_times[guard][0]:
			for i in range(sleep_session[0], sleep_session[1]):
				points[i] = '#'
		guard_sleep_times[guard] = (guard_sleep_times[guard][0], points)
	
	for guard, sleep_time_tuples in guard_sleep_times.iteritems():
		data.append(day + '  ' + guard + '  ' + ''.join(sleep_time_tuples[1]))

# Parse space-based data into records (dictionary per guard/date)
records = parse_records(data)
guard_data = aggregate_per_guard(records)

# Find the guard that has the most minutes asleep
sleepy_guard = get_who_slept_most(guard_data)

# What minute does that guard spend asleep the most?
sleepy_minute = get_sleepiest_minute(guard_data[sleepy_guard])

# Id * Minute
result = int(sleepy_guard.replace('#', '')) * sleepy_minute
print('Sleepiest guard: ' + sleepy_guard, 'Sleepiest minute: ' + str(sleepy_minute), 'Result: ' + str(result))
